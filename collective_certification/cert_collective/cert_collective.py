...
def calc_local_budgets(local_budget_descriptor, local_budget_mode, attr, adj):
    ...
    if local_budget_mode == "relative":
        ...
        local_budgets = np.floor(
            np.minimum(local_budget_descriptor[:, np.newaxis], potential_change)
        )
    else:
...
def calc_sparse_directed_incidence_masks(edge_idx, n_nodes):
    ...
    return (
        sp.sparse.csr_matrix(mask[0]),
        sp.sparse.csr_matrix(mask[1]),
    )
...
def calc_grid_pareto_points(grid):
    ...
    for node in range(grid.shape[0]):
        ...
        )  # low-norm points should dominate many others
...
def generate_collective_problem(
    n_nodes,
    n_edges,
    dim_labels,
    receptive_field_masks,
    pareto_points,
    pareto_points_node_mask,
    local_budgets=None,
    ...
    pareto_point_dim_indicators = cp.Variable(
        (perceived_perturbs.shape[0], num_pareto), pos=True
    )
    ...
    constraints = [
        ...
        *[
            pareto_point_indicators <= pareto_point_dim_indicators[d]
            for d in range(n_dims)
        ],
        pareto_points_node_mask
        @ cp.multiply(pareto_point_indicators, pareto_points_reachable_mask)
        >= attacked,
        attacked <= 1,
        perceived_perturbs @ pareto_points_node_mask
        >= cp.multiply(pareto_point_dim_indicators, pareto_points.T),
    ]
        ...
        if num_attackers is not None:
            ...
            constraints.extend(
                [
                    ...
                    (
                        (
                            sparse_directed_incidence_masks[0]
                            + sparse_directed_incidence_masks[1]
                        )
                        @ edge_del
                        <= local_budgets[0]
                    ),
...
def eval_collective_cert_problem(
    problem,
    radius_param,
    pareto_points_reachable_param,
    radius,
    pareto_points,
    dim_labels,
    n_nodes,
    ...
    pareto_points_reachable_mask = calc_pareto_points_reachable_mask(
        pareto_points, dim_labels, radius
    )
    ...
    if solver == "MOSEK":
        ...
        num_attacked = problem.solve(solver=solver, ignore_dpp=True)
...
def collective_certificate_grid(
    base_grid,
    dim_labels,
    receptive_field_masks,
    attr,
    ...
    if local_budget_descriptor is not None:
        local_budgets = calc_local_budgets(
            local_budget_descriptor, local_budget_mode, attr, adj
        )
        sparse_directed_incidence_masks = calc_sparse_directed_incidence_masks(
            edge_idx, n_nodes
        )
    else:
        local_budgets = None
    ...
    pareto_points_reachable_mask = calc_pareto_points_reachable_mask(
        pareto_points, dim_labels, max_rad
    )
    pareto_points = pareto_points[pareto_points_reachable_mask != 0]
    pareto_points_node_mask = pareto_points_node_mask[
        :, pareto_points_reachable_mask != 0
    ]
    ...
    cert_grid = np.empty(
        (len(edge_del_vals), len(attr_add_vals), len(attr_del_vals)), dtype="int"
    )
    ...
    for edge_del, attr_add, attr_del in tqdm(list(itertools.product(
        edge_del_vals, attr_add_vals, attr_del_vals
    ))):
        radius = np.array([edge_del, attr_add, attr_del])

        num_robust = eval_collective_cert_problem(
            problem,
            radius_param,
            pareto_points_reachable_param,
            radius,
        ...
        cert_grid[
            edge_del_vals == edge_del,
            attr_add_vals == attr_add,
            attr_del_vals == attr_del,
        ] = num_robust
